...
def scrapeVideo(threads: int = Query(10, description="Number of threads to use for scraping"),
                retries: int = Query(3, description="Number of retries for each video URL"),
                scrapeCount: int = Query(50, description="Number of comments to scrape per request"),
                videoURLS: VideoURLS = Body(..., description="List of video URLs to scrape comments from")) -> Dict[str, Dict[str, List[str]]]:
    """
    API to scrape comments from a list of video URLs scraping approximately 100±10 comments per second
    ...
    with ThreadPoolExecutor(max_workers=threads) as executor:
        futures = [executor.submit(scrapeManager, link, scrapeCount, retries) for link in videoURLS.URLS]
...
def ntu():
    return {"text": 
"""If you are contemplating NTU, please seek help
...
# Helper Functions
def scrapeHandler(awemeID: str, scrapeCount: int, curr: int) -> tuple[list[str], bool, bool]:
...
def scrapeManager(videoLink: str, scrapeCount: int, retryCount: int) -> Dict[str, List[str]]:
